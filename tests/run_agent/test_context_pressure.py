...
class TestContextPressureFlags:
    ...
    def test_compression_flush_flag_defaults_off_from_config(self):
        with (
    ...
    def test_non_dict_config_falls_back_to_safe_defaults(self):
        with (
    ...
    def test_memory_store_receives_project_memory_config(self):
        with (
            patch(
    ...
    def test_invoke_memory_tool_forwards_scope_ttl_and_working_dir(self, agent):
    ...
    def test_project_scoped_memory_does_not_mirror_to_external_provider(self, agent):
        agent._memory_store = MagicMock()
    ...
    def test_sync_dynamic_session_context_rebuilds_prompt_after_repo_change(self, agent):
        agent._memory_store = MagicMock()
    ...
    def test_sync_dynamic_session_context_skips_rebuild_when_cwd_is_unchanged(self, agent):
        agent._memory_store = MagicMock()
    ...
    def test_compression_flush_flag_reads_truthy_config(self):
        with (
            patch("hermes_cli.config.load_config", return_value={"memory": {"flush_on_compress": "yes"}}),
            patch("run_agent.get_tool_definitions", return_value=_make_tool_defs("web_search")),
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
        ):
        ...
        assert a._compression_memory_flush_enabled is True
    ...
    def test_compression_skips_memory_flush_when_disabled(self, agent):
        """Lean mode should not auto-save memory during compression unless enabled."""
        agent._compression_memory_flush_enabled = False
        agent.context_compressor = MagicMock()
        agent.context_compressor.compress.return_value = [
            {"role": "user", "content": "Summary of conversation so far."}
        ]
        ...
        agent.flush_memories.assert_not_called()

    def test_compression_flushes_memory_when_enabled(self, agent):
        """Compression may still flush memory when the config explicitly enables it."""
        agent._compression_memory_flush_enabled = True
        agent.context_compressor = MagicMock()
        agent.context_compressor.compress.return_value = [
            {"role": "user", "content": "Summary of conversation so far."}
        ]
        ...
        agent.flush_memories.assert_called_once_with(messages, min_turns=0)

    def test_compression_skips_external_memory_provider_flush_when_disabled(self, agent):
        """Lean mode should also skip provider-side pre-compression persistence."""
        agent._compression_memory_flush_enabled = False
        agent.context_compressor = MagicMock()
        agent.context_compressor.compress.return_value = [
            {"role": "user", "content": "Summary of conversation so far."}
    ...
    def test_compression_calls_external_memory_provider_flush_when_enabled(self, agent):
        """Provider-side pre-compression hooks remain available when opted in."""
        agent._compression_memory_flush_enabled = True
        agent.context_compressor = MagicMock()
        agent.context_compressor.compress.return_value = [
            {"role": "user", "content": "Summary of conversation so far."}
