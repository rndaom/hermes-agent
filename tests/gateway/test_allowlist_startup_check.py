...
def _would_warn():
    ...
    return not _three_ds_enabled and not _any_allowlist and not _allow_all
...
class TestAllowlistStartupCheck:
    ...
    def test_threeds_only_setup_suppresses_warning(self):
        with patch.dict(os.environ, {"THREEDS_ENABLED": "true"}, clear=True):
            assert _would_warn() is False
